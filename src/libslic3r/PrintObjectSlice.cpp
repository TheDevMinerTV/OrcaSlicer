...
// Moves the outer "shell material" band (config: shell_material_filament_id + shell_material_thickness) of every
// layer into the shell material regions generated by generate_print_object_regions().
// The band is a cylindrical 3D erosion of the object: a point stays in the core only if it has at least
// shell_material_thickness clearance to the object surface both horizontally (XY offset of each layer outline)
// and vertically (every layer within the thickness window must cover it with its eroded outline; layers missing
// beyond the object top/bottom count as empty, thus the top/bottom band becomes shell as well).
template<typename ThrowOnCancel>
static void apply_shell_material_segmentation(PrintObject &print_object, ThrowOnCancel throw_on_cancel)
{
    const int num_layers = int(print_object.layer_count());
    if (num_layers == 0)
        return;
    ...
    // Whole-layer outlines. Layer::lslices are not built yet at this point. The multi-material segmentation
    // leaves hairline slits between neighboring regions; close them, or the erosion below would carve
    // channels along the region boundaries. The closing keeps the outer boundary unchanged.
    std::vector<ExPolygons> merged(num_layers);
    tbb::parallel_for(tbb::blocked_range<int>(0, num_layers), [&print_object, &merged, throw_on_cancel](const tbb::blocked_range<int> &range) {
        for (int layer_idx = range.begin(); layer_idx < range.end(); ++layer_idx) {
            throw_on_cancel();
            merged[layer_idx] = closing_ex(print_object.get_layer(layer_idx)->merged(0.f), scaled<float>(10. * EPSILON));
        }
    });
    ...
    for (const auto &[thickness, include_top_bottom] : band_specs) {
        ...
        tbb::parallel_for(tbb::blocked_range<int>(0, num_layers), [&merged, &eroded, thickness = thickness, throw_on_cancel](const tbb::blocked_range<int> &range) {
            for (int layer_idx = range.begin(); layer_idx < range.end(); ++layer_idx) {
                throw_on_cancel();
                eroded[layer_idx] = offset_ex(merged[layer_idx], -float(scale_(thickness)));
            }
        });
    ...
    tbb::parallel_for(tbb::blocked_range<int>(0, num_layers, std::max(num_layers / 128, 1)), [&print_object, &bands, &strips, &strip_width, throw_on_cancel](const tbb::blocked_range<int> &range) {
        ...
        // Maps print_object_region_id -> print_object_region_id of the shell material region stealing from it, -1 for none.
        ...
        for (int layer_idx = range.begin(); layer_idx < range.end(); ++layer_idx) {
            ...
            Layer &layer = *print_object.get_layer(layer_idx);
            it_layer_range = layer_range_next(layer_ranges, it_layer_range, layer.slice_z);
            const PrintObjectRegions::LayerRangeRegions &layer_range = *it_layer_range;
            if (layer_range.shell_material_regions.empty())
                continue;

            if (&layer_range != cached_layer_range) {
                ...
                for (const PrintObjectRegions::ShellMaterialRegion &shell_region : layer_range.shell_material_regions) {
                    const PrintRegion *parent = layer_range.volume_regions[shell_region.parent].region;
                    // Don't trim by self (the shell config may deduplicate into the parent region).
                    if (shell_region.region == parent)
                        continue;
                    const int parent_id = parent->print_object_region_id();
                    const int shell_id  = shell_region.region->print_object_region_id();
                    // The core-side region of this parent (shell_material_interface_wall_loops override), if any.
                    int core_id = -1;
                    ...
                    // regions exist for this parent, only they are shelled — their band becomes the shell region and
                    // their remainder returns to the core material. Unpainted areas are left alone.
                    // With scope == WholeObject the paint does not mask: the painted content is redistributed the
                    // same way, but the rest of the object is shelled as well, keeping a single continuous core.
                    const int shell_filament = std::min(parent->config().shell_material_filament_id.value, int(num_filaments));
                    const bool allow_paint_mask = parent->config().shell_material_scope.value == ShellMaterialScope::Auto;
                    bool masked = false;
                    for (const PrintObjectRegions::PaintedRegion &painted_region : layer_range.painted_regions)
                        if (int(painted_region.extruder_id) == shell_filament &&
                            layer_range.volume_regions[painted_region.parent].region == parent) {
                            const int painted_id = painted_region.region->print_object_region_id();
                            if (painted_id != shell_id && painted_id != parent_id) {
                                target[painted_id] = shell_id;
                                // In the paint-masked mode the painted remainder joins the parent region directly:
                                // routing it into the core-side region would print a needless double wall where the
                                // painted core meets the untouched parent core (both are the same material).
                                // With whole-object scope the parent remainder moves to the core-side region too,
                                // so both remainders converge in a single region either way.
                                core_target[painted_id] = ! allow_paint_mask && core_id >= 0 ? core_id : parent_id;
                                masked = allow_paint_mask;
                            }
                        }
                    parent_masked[parent_id] = masked;
                    if (! masked) {
                        target[parent_id] = shell_id;
                        if (core_id >= 0)
                            core_target[parent_id] = core_id;
                    }
                }
                // Fuzzy skin variants of plain volume regions carry no paint and follow their (unmasked) parent.
                // Regions painted with other filaments are never stolen from: their paint wins inside the band.
                for (const PrintObjectRegions::FuzzySkinPaintedRegion &fuzzy_skin_region : layer_range.fuzzy_skin_painted_regions) {
                    if (fuzzy_skin_region.parent_type != PrintObjectRegions::FuzzySkinPaintedRegion::ParentType::VolumeRegion)
                        continue;
                    const int parent_id = fuzzy_skin_region.parent_print_object_region_id(layer_range);
                    const int self_id   = fuzzy_skin_region.region->print_object_region_id();
                    if (! parent_masked[parent_id] && target[parent_id] >= 0 && target[parent_id] != self_id && self_id != parent_id) {
                        target[self_id]      = target[parent_id];
                        core_target[self_id] = core_target[parent_id] >= 0 && core_target[parent_id] != self_id ? core_target[parent_id] : -1;
                    }
                }
            }

            // Seed every region with its current content, so that regions untouched by the shell band
            // (including the shell regions themselves) are preserved by the final re-bucketing.
            std::vector<ByRegion> by_region(layer.region_count());
            for (int region_id = 0; region_id < layer.region_count(); ++region_id)
                by_region[region_id].expolygons = to_expolygons(layer.get_region(region_id)->slices.surfaces);

            for (int region_id = 0; region_id < layer.region_count(); ++region_id) {
                const int target_region_id = target[region_id];
                ...
                ExPolygons mine   = std::move(by_region[region_id].expolygons);
                ExPolygons stolen = intersection_ex(mine, band);
                if (! stolen.empty()) {
                    // Smooth the shell boundary: the painted / Voronoi-segmented sources have jagged edges, and
                    // when both regions' walls trace them independently, they leave hairline gaps between the
                    // materials. Smooth the stolen side, then derive the remainder as its exact complement so the
                    // two regions always share a single clean contour.
                    const float smooth_radius = scaled<float>(0.25);
                    stolen = intersection_ex(closing_ex(opening_ex(stolen, smooth_radius), smooth_radius), mine);
                }
                Polygons remaining = diff(to_polygons(mine), stolen);
                // Filter out unprintable polygons. Detailed explanation is inside apply_mm_segmentation.
                if (! remaining.empty())
                    remaining = opening(union_ex(remaining), scaled<float>(5. * EPSILON), scaled<float>(5. * EPSILON));
                by_region[region_id].expolygons.clear();

                // The remainder either stays in the source region or moves to the core-side region
                // overriding the wall count at the material interface.
                const int keep_region_id = core_target[region_id] >= 0 ? core_target[region_id] : region_id;
                if (! remaining.empty()) {
                    ByRegion &dst = by_region[keep_region_id];
                    if (dst.expolygons.empty()) {
                        dst.expolygons = union_ex(remaining);
                    } else {
...
void PrintObject::slice_volumes()
{
    ...
    // Does any region print an outer shell material band?
    const bool has_shell_material_regions = std::any_of(
        this->shared_regions()->layer_ranges.begin(), this->shared_regions()->layer_ranges.end(),
        [](const PrintObjectRegions::LayerRangeRegions &layer_range) { return ! layer_range.shell_material_regions.empty(); });
    if (has_shell_material_regions) {
        // If XY Size compensation is also enabled, notify the user that XY Size compensation
        // would not be used because it would skew the shell material band computed here.
        if (m_config.xy_hole_compensation.value != 0.f || m_config.xy_contour_compensation.value != 0.f) {
            this->active_step_add_warning(
                PrintStateBase::WarningLevel::CRITICAL,
                L("An object's XY size compensation will not be used because it prints a shell material band.\nXY Size "
                  "compensation cannot be combined with shell material."));
        }

        BOOST_LOG_TRIVIAL(debug) << "Slicing volumes - shell material segmentation";
        apply_shell_material_segmentation(*this, [print]() { print->throw_if_canceled(); });
    }
    ...
    BOOST_LOG_TRIVIAL(debug) << "Slicing volumes - make_slices in parallel - begin";
    {
        ...
        const bool   suppress_xy_compensation = (num_extruders > 1 && this->is_mm_painted()) || has_shell_material_regions;
        const auto   xy_hole_scaled = suppress_xy_compensation ? scaled<float>(0.f) : scaled<float>(m_config.xy_hole_compensation.value);
        const auto   xy_contour_scaled            = suppress_xy_compensation ? scaled<float>(0.f) : scaled<float>(m_config.xy_contour_compensation.value);
